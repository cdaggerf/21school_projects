...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/10/18 14:12:09 by cdaggerf          #+#    #+#             */
/*   Updated: 2021/10/18 21:55:11 by cdaggerf         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int	ft_len_xxx(unsigned long num)
{
	int	l;
...
static int	ft_pos_prec_xxx(t_flags *flags, int l_x, int l, char *str)
{
...
static int	ft_neg_prec_xxx(t_flags *flags, int l_x, int l, char *str)
{
...
int	print_xxx(unsigned int num, t_flags *flags)
{
	unsigned long	nbr;
	int				l;
	int				l_x;
	...
	str = ft_itoa_xxx(nbr);
	if (flags->precis == -1)
		l += ft_neg_prec_xxx(flags, l_x, l, str);
	if (flags->precis >= 0)
		l += ft_pos_prec_xxx(flags, l_x, l, str);
	free(str);
	return (l + l_x);
}
